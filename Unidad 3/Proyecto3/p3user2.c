...
void cerrarmemoria(){ //AQUI CIERRO LA MEMORIA RESERVADA
    ...
    if(shm_unlink("shm2")<0){ //Cerramos el link del archivo
...
void encender(){ //Aqui encendemos los semaforos

//Abrimos los semaforos que fueron creados por el otro usuario
    semf1 = sem_open("semaforo1", 0); 
    while(semf1== SEM_FAILED){
        sleep(1); //Lo duerme unr ato para que no se ejecute la instruccion demasiadas veces mientras el otro usuario se conecta y crea los semaforos
        semf1 = sem_open("semaforo1", 0); 
    }
    semf2 = sem_open("semaforo2", 0); 
    if(semf2== SEM_FAILED){
        //CREAMOS LOS SEMAFOROS 2-4 QUE SON LOS DEL USER 2 
        mode_t permisos = 0666;
        int banderas = O_CREAT;
        unsigned int modo =0; 
        semf2 = sem_open("semaforo2", banderas, permisos, modo); //CREAMOS EL SEMAFORITO
        if(semf2 == SEM_FAILED){
...
while (1)
{
    //Primero antes de mapear, debemos truncar la memoria, lo cual ya esta hecho, en el main (EN EL DE RESERVAR MEMORIAS)
...
void *msgleer(){
    ...
    while (1)
    {
        ...
        fprintf(stdout,"Usuario 1 dice: %s", msgbuf);
